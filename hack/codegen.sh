...
for gv in "${apiGroups[@]}"; do
  docker run --rm -ti -u $(id -u):$(id -g) \
    ...
    --input-dirs "$PACKAGE_NAME/apis/${gv},k8s.io/apimachinery/pkg/apis/meta/v1,k8s.io/apimachinery/pkg/api/resource,k8s.io/apimachinery/pkg/runtime,k8s.io/apimachinery/pkg/util/intstr,k8s.io/apimachinery/pkg/version,k8s.io/api/core/v1,k8s.io/api/apps/v1,github.com/tekliner/monitoring-agent-api/api/v1,github.com/tekliner/objectstore-api/api/v1,github.com/tekliner/offshoot-api/api/v1,github.com/appscode/go/encoding/json/types,github.com/tekliner/custom-resources/apis/appcatalog/v1alpha1,k8s.io/api/rbac/v1" \
